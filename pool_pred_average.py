...
def meta_cnn_train(name, model_file, base_folder, pool_folder):
    '''
    This function is to train the meta-model based CNN
    '''
    
    x1_pool = pd.read_csv(os.path.join( base_folder, 'train_1',  "all_stacked_pool_prob.csv"  ), index_col=0)
    x2_pool = pd.read_csv(os.path.join( base_folder, 'train_2',  "all_stacked_pool_prob.csv"  ), index_col=0)
    x3_pool = pd.read_csv(os.path.join( base_folder, 'train_3',  "all_stacked_pool_prob.csv"  ), index_col=0)
    x4_pool = pd.read_csv(os.path.join( base_folder, 'train_4',  "all_stacked_pool_prob.csv"  ), index_col=0)
    x5_pool = pd.read_csv(os.path.join( base_folder, 'train_5',  "all_stacked_pool_prob.csv"  ), index_col=0)
    x6_pool = pd.read_csv(os.path.join( base_folder, 'train_6',  "all_stacked_pool_prob.csv"  ), index_col=0)
    y_pool = pd.read_csv(os.path.join( pool_folder, "y_pool.csv"  ), index_col=0)
...
def main():
    base_folder = input("Enter the base folder (e.g., subset1): ").strip()
    pool_folder = input("Enter the pool folder (e.g., data/initial_pool): ").strip()
    meta_folder = input("Enter the meta folder name (e.g., meta_pool_cnn): ").strip()

    os.makedirs(meta_folder, exist_ok=True)
    name = f"{base_folder}/{meta_folder}"
    os.makedirs(name, exist_ok=True)
    print("#" * 100)
    print(name)
    meta_cnn_train(name, model_file=f"{base_folder}/meta_average_cnn", base_folder=base_folder, pool_folder=pool_folder)
    print("finish train", name, base_folder, pool_folder)
