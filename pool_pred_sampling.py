...
def main():
    ...
    pool_file = input("Enter the pool file folder (e.g., data/initial_pool): ").strip()
    num_folders = 6 #int(input("Enter the number of train folders (e.g., 6): ").strip())

    for i in range(1, num_folders + 1):
        folder_name = f'{base_folder}/train_{i}'
        if not os.path.exists(folder_name):
            os.makedirs(folder_name)
            print(f"Folder '{folder_name}' created.")
        print("#" * 100)
        print(folder_name)
        load(folder_name, pool_file=pool_file, model_file=folder_name)
